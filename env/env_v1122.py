...
class BaseEnvironment(gym.Env):
    """MDP environment of single-BS, version 1.12.2.
    ...
    def reset(
        ...
        # Load the selected map and calculate the action mask
        ...
        # Initialize the state: no TX (transmitter) deployed
        self.tx_locs = []
        self.r_prev = 0.
    ...
    def _normalize_rewards(self, reward, min_value, max_value):
        """
        Normalize the reward values to the range [0, 1].
        
        :param reward: numpy array of reward values
        :param min_value: minimum value of the reward
        :param max_value: maximum value of the reward
        :return: normalized reward values
        """
        return np.clip((reward - min_value) / (max_value - min_value), 0, 1)
    ...
    def calc_coverage(self, tx_locs: list) -> tuple[np.ndarray, int]:
        ...
        _, avg_capacity = self.calc_capacity_reward(tx_locs)
        ...
        for tx_loc in tx_locs:
            ...
            power_map[self.pixel_map != self.non_building_pixel] = 0 # ROI should be black
        ...
        avg_coverage = overall_coverage[overall_coverage==1].sum() / num_roi
        
        # Normalize capacity and coverage
        normalized_avg_capacity = self._normalize_rewards(avg_capacity, 0.5, 0.65)
        normalized_avg_coverage = self._normalize_rewards(avg_coverage, 0.08, 0.94)
        
        # Combine both rewards
        reward = normalized_avg_capacity * 1 + normalized_avg_coverage * 1
        
        return overall_coverage, float(reward)
