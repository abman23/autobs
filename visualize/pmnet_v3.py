...
def get_output(model, inputs, transform = transform):
    inputs = transform(inputs).type(torch.float32).cuda()
    preds = model(torch.stack((inputs, inputs))) #since the model expects a batch input
    preds = torch.clip(preds, 0, 1)
    output = preds[0, 0].cpu().detach().numpy()
    return output
